...
while IFS=';' read -r id isoCode title titleEn status
  do
    ...
    if [ "$id" != "id" ]; then
      ...
      language_exists=$(psql -h $db_host -p $db_port -d $db_name -U $db_user -t -c "SELECT 1 FROM \"$db_name\".\"Dictionaries\".\"Languages\" WHERE \"Id\" = $id;" > /dev/null)
      ...
      if [ -z "$language_exists" ]; then
        ...
        sql="INSERT INTO \"$db_name\".\"Dictionaries\".\"Languages\" 
        ...
        if psql -h $db_host -p $db_port -d $db_name -U $db_user -c "$sql" > /dev/null; then
          echo "Language with ID $id added successfully."
        else
          ((errorAdded++))
          echo "Error adding language with ID $id." >> "$log_file"
        fi
      else
        ((alreadyExist++))
